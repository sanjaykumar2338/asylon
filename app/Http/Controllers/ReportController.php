...
class ReportController extends Controller
{
    ...
    {
        if ($forwarded = $this->forwardedDashboardUrl($request)) {
            return $forwarded;
        }

        if ($serverDerived = $this->serverDerivedDashboardUrl()) {
    ...
    protected function forwardedDashboardUrl(StoreReportRequest $request): ?string
    {
        $fromForwarded = $this->forwardedHeaderUrl($request->header('Forwarded'));

        if ($fromForwarded) {
            return $fromForwarded;
        }

        $host = $this->firstForwardedValue($request->header('X-Forwarded-Host'));

        if ($host === null) {
            return null;
        }

        $scheme = $this->firstForwardedValue($request->header('X-Forwarded-Proto')) ?? $request->getScheme() ?? 'https';
        $port = $this->firstForwardedValue($request->header('X-Forwarded-Port'));

        if ($port !== null && ! str_contains($host, ':')) {
            $host .= ':'.$port;
        }

        return rtrim(sprintf('%s://%s', $scheme, $host), '/');
    }

    protected function serverDerivedDashboardUrl(): ?string
    {
        $host = $this->firstForwardedValue($_SERVER['HTTP_X_FORWARDED_HOST'] ?? null)
            ?? $this->sanitizeHost($_SERVER['HTTP_HOST'] ?? null)
            ?? $this->sanitizeHost($_SERVER['SERVER_NAME'] ?? null);

        if ($host === null) {
            return null;
        }

        $scheme = $this->firstForwardedValue($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? null)
            ?? ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http');

        $port = $this->firstForwardedValue($_SERVER['HTTP_X_FORWARDED_PORT'] ?? null)
            ?? ($_SERVER['SERVER_PORT'] ?? null);

        if ($port !== null && ! str_contains($host, ':')) {
            $portInt = is_numeric($port) ? (int) $port : $port;

            if (! in_array($portInt, [80, 443], true)) {
                $host .= ':'.$port;
            }
        }
    ...
    protected function forwardedHeaderUrl(?string $header): ?string
    {
        ...
        $segments = array_filter(array_map('trim', explode(';', $first)));
    ...
    protected function sanitizeHost(?string $value): ?string
    {
