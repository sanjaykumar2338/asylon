<x-admin-layout>
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-3 admin-page-header">
            <div>
                <h1 class="h4 mb-1">{{ __('Report Details') }}</h1>
                <p class="text-muted mb-0 small">{{ __('Review the submission, update status, and manage communication.') }}</p>
            </div>
            <div class="d-flex flex-wrap align-items-center justify-content-end" style="gap: 0.5rem;">
                <span class="text-muted text-monospace small">#{{ $report->id }}</span>
                @can('manage-data-requests')
                    <form method="POST" action="{{ route('admin.data_requests.from_case', $report) }}" class="mb-0">
                        @csrf
                ...
                <form method="POST" action="{{ route('reports.destroy', $report) }}"
                    ...
                    data-swal-icon="warning" class="mb-0">
                    @csrf
                    @method('DELETE')
        ...
        <div class="row g-3">
            <div class="col-xl-8">
                <div class="card admin-index-card mb-3">
                    <div class="card-body">
                        <div class="d-flex flex-wrap align-items-center justify-content-between mb-3" style="gap: 0.35rem;">
                            <div class="d-flex flex-wrap align-items-center" style="gap: 0.35rem;">
                                <span class="badge badge-secondary text-uppercase">{{ $report->category }}</span>
                                @if ($report->subcategory)
                                    <span class="badge badge-light text-dark text-uppercase">{{ $report->subcategory }}</span>
                                @endif
                                <span class="badge badge-info text-capitalize">{{ str_replace('_', ' ', $report->status) }}</span>
                                @if ($report->urgent)
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Category') }}</div>
                                <div>{{ $report->category }}</div>
                                <div class="text-muted small">{{ $report->subcategory ?? __('Not provided') }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Severity') }}</div>
                                <div class="text-capitalize">{{ $report->severity_label }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Organization') }}</div>
                                <div>{{ $report->org?->name ?? __('Unknown') }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Submitted') }}</div>
                                <div>{{ $report->created_at->format('M d, Y H:i') }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Violation date') }}</div>
                                <div>{{ $report->violation_date?->format('M d, Y') ?? __('Not provided') }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('First response') }}</div>
                                <div>{{ $report->first_response_at?->format('M d, Y H:i') ?? __('Not yet') }}</div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Reporter chat') }}</div>
                            ...
                            <div class="col-md-6">
                                <div class="text-muted text-uppercase small mb-1">{{ __('Type') }}</div>
                                <div class="text-capitalize">{{ $report->type_label }}</div>
                            </div>
                        </div>

                        @if ($report->escalationEvents->isNotEmpty())
                            <div class="mt-4">
                                <h6 class="text-uppercase small text-muted mb-2">{{ __('Escalation') }}</h6>
                        ...
                        <div class="mt-4">
                            <h6 class="text-uppercase small text-muted mb-2">{{ __('Description') }}</h6>
                        ...
                        <div class="mt-4">
                            <h6 class="text-uppercase small text-muted mb-2">{{ __('Reporter Contact') }}</h6>
                            <div class="row">
                                <div class="col-md-4 mb-2"><strong>{{ __('Name') }}:</strong> {{ $report->contact_name ?? __('Not provided') }}</div>
                                <div class="col-md-4 mb-2"><strong>{{ __('Email') }}:</strong> {{ $report->contact_email ?? __('Not provided') }}</div>
                                <div class="col-md-4 mb-2"><strong>{{ __('Phone') }}:</strong> {{ $report->contact_phone ?? __('Not provided') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
